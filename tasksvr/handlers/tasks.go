...
import (
	...
	"github.com/bond00729/info344-in-class/tasksvr/models/tasks"
)
...
func (ctx *Context) TasksHandler(w http.ResponseWriter, r *http.Request) {
	...
	case "GET":
		...
		if err != nil {
			http.Error(w, "error getting tasks: %v", http.StatusInternalServerError)
			return
		}
		respond(w, tasks)
	case "POST":
		...
		if err := json.NewDecoder(r.Body).Decode(nt); err != nil {
			http.Error(w, fmt.Sprintf("error posting task: %v", err), http.StatusBadRequest)
			return
		}
